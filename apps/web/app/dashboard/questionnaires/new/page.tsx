  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/questionnaires", {
        ...
        body: JSON.stringify({ name, riskTier, questions }),
      });
      if (!res.ok) {
    ...
    <main style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ background: "var(--panel)", borderBottom: "1px solid var(--border)", padding: "0 32px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ color: "var(--border)", fontSize: 16 }}>/</span>
          <span style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 500 }}>New</span>
        </div>
      </div>

      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 32px" }}>
        <h1 style={{ fontSize: "1.5rem", marginBottom: 28 }}>New questionnaire</h1>

        {/* Meta */}
        <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 8, padding: "24px", marginBottom: 24 }}>
          <div style={{ marginBottom: 16 }}>
            <label style={labelStyle}>Template name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Standard KYC — Low Risk" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Risk tier</label>
            <div style={{ display: "flex", gap: 8 }}>
              {(["LOW", "MEDIUM", "HIGH"] as const).map((t) => (
                <button key={t} onClick={() => setRiskTier(t)} style={{ padding: "7px 20px", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer", background: riskTier === t ? "var(--brand)" : "var(--panel-raised)", color: riskTier === t ? "#fff" : "var(--text)", border: `1.5px solid ${riskTier === t ? "var(--brand)" : "var(--border)"}` }}>
                  {t}
                </button>
              ))}
            </div>
    ...
    <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 8, padding: "18px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <input value={q.field} onChange={(e) => onChange({ field: e.target.value.replace(/\s/g, "_") })} placeholder="e.g. source_of_funds" style={inputStyle} />
        </div>
        <div>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={labelStyle}>Label (shown in web UI)</label>
        <input value={q.label} onChange={(e) => onChange({ label: e.target.value })} placeholder="What is your occupation?" style={inputStyle} />
      </div>

      <div style={{ marginBottom: 12 }}>
        <label style={labelStyle}>Voice prompt (how Vera asks it)</label>
        <input value={q.voicePrompt} onChange={(e) => onChange({ voicePrompt: e.target.value })} placeholder="Can you tell me your current occupation?" style={inputStyle} />
      </div>
